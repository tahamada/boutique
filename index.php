...
//nomnbre d'article lié a la recherche
$nbArticle=$mArticle::lister($search,array("count"),$objet,$joinParam,$order);
...
if(isset($_GET['page']) && is_numeric($_GET['page']) && intval($_GET['page'])>0){
	$offset=(intval($_GET['page'])-1)*intval($limit);
	$page=$_GET['page'];
}
//liste des aricle souhaité
$listArticle=$mArticle::lister($search,$column,$objet,$joinParam,$order,$limit,$offset);
...
if(isset($_GET['mode']) && $_GET['mode']=="ajax"){
	...
	echo $twig->render('accueil.html',array('categorie'=>$listeoCategorie,'articlesSlide'=>$listArticleNouveau,'articles'=>$listArticle,'message'=>$message,'session'=>$session,'nbArticle'=>$nbArticle['nb'],'page'=>$page,'limit'=>$limit,'idClient'=>$idClient));
}
?>